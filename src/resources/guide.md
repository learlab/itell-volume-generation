...
- Images: `![caption or description](image_page_X_Y)` - ALWAYS include all images with captions
...
- Multimedia elements like images should be represented using markdown image syntax: `![caption](image_page_X_Y)`
- **ALWAYS include all images that have captions** - they are essential content
...
Pages in iTELL have two main fields that must be populated:
...
- Exclude any summaries of the page.
- **ALWAYS include images with captions using proper markdown image syntax: `![caption](image_page_X_Y)`**
- Exclude chunks that have review questions which have the same functions as iTELL's Question and ConstructedResponse pair.

## Image Handling

When image metadata is provided, **ALL images with captions MUST be included** in the JSON output.
...
Where:
- `image_page_X_Y` is the image_id from metadata (e.g., image_page_2_1, image_page_5_3)
- Text in brackets `[...]` is the full caption if available, or a brief description
...
```json
{
  "Text": "Photosynthesis occurs in chloroplasts.\n\n![Figure 2.1: Cross-section of a chloroplast showing thylakoid membranes](image_page_5_1)\n\nThe process involves light-dependent reactions."
}
```

**Incorrect formats to avoid:**

- `{{image_page_5_1}}` ❌ (Not standard Markdown)
- `<img src="image_page_5_1">` ❌ (HTML not allowed)
- Text reference: "see Figure 1" ❌ (Use actual image)
- `[IMAGE]` or `[IMAGE: description]` ❌ (Placeholder not allowed)

### Image Inclusion Rules

- **ALL images with captions** → MUST include
- Diagrams, charts, figures → MUST include
- Tables or equations as images → MUST include
- Photos/illustrations → Include if content-relevant
- Decorative elements/logos → May skip
...
3. **Include ALL images with captions**: Use standard Markdown syntax `![caption](image_page_X_Y)` - never skip captioned images
4. **Maintain consistent structure**: All pages in Pages array, all chunks in Content array
5. **Apply Title Case consistently**: All headers and titles should use Title Case
6. **Balance chunk types**: Use page.plain-chunk for supplementary material (objectives, references), page.chunk for main content
7. **Keep chunks focused**: 200-400 words per chunk, centered on a single topic
8. **Write natural keyphrases**: Pull directly from text, use noun phrases
9. **Create clear Q&A pairs**: Questions should be specific, answers should be concise (1-2 sentences)
10. **Validate JSON structure**: Ensure proper nesting, commas, and brackets
...
3. **Missing images with captions**: All captioned images from metadata must be included
4. **Wrong image format**: Use `![caption](image_page_X_Y)`, not `{{image_page_X_Y}}` or HTML
5. **Text references instead of images**: Don't use "see Figure 1" - use actual markdown images
6. **Malformed Markdown**: Missing blank lines between paragraphs, incorrect bold/italic syntax
7. **Overly long chunks**: Keep chunks under 500 words
8. **Generic keyphrases**: Avoid vague terms like "important concepts" or "key ideas"
9. **Yes/no questions**: Questions should require substantive answers
10. **Missing Title/Description/VolumeSummary**: These are required at volume level
11. **Inconsistent capitalization**: Use Title Case for all headers
12. **Bullet points in VolumeSummary**: Use complete, flowing sentences instead